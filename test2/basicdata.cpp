...
Matrix3d CellVector(const BasicData &data)
{
    ...
    // 1 list a = a*(1, 0, 0)
    Cell(0, 0) = data._cell_length_a;
    Cell(1, 0) = 0.0;
    Cell(2, 0) = 0.0;

    // 2 list b = b*(cos(γ), sin(γ), 0 )
    Cell(0, 1) = data._cell_length_b * cg;
    Cell(1, 1) = data._cell_length_b * sg;
    Cell(2, 1) = 0.0;

    // 3 list c = c* (cos(β), (cos(α) - cos(β)*cos(γ)) / sin(γ),
    //                1 + 2*cos(α)*cos(β)*cos(γ)- cos(α)*cos(α)
    //                - cos(β)*cos(β)- cos(γ)*cos(γ)) / sin(γ))
    Cell(0, 2) = data._cell_length_c * cb;
    Cell(1, 2) = data._cell_length_c * (ca - cb * cg) / sg;
    Cell(2, 2) = data._cell_length_c * std::sqrt(1.0 + 2.0 * ca * cb * cg - ca * ca - cb * cb - cg * cg) / sg;
